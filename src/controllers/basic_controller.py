...
class BasicMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False,training=False,learner=None,execute=False):
        ...
        if(learner!=None and execute==True):
            ...
            mone = th.zeros_like(q_temp).numpy()
            mechane = th.zeros_like(q_temp).numpy()
            for a in all_tensors:
                a_numpy = a.numpy()
                #mone+=th.pow(a_numpy-q,4)
                #mechane+=th.pow(a_numpy-q,2)
                mone+=np.power(a_numpy-q,4)
                mechane+=np.power(a_numpy-q,2)
